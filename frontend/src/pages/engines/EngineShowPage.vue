<template>
  <section class="section">
    ...
    <div v-if="result">
      <h1 class="title">{{ result.engine.name }}</h1>

      <p v-if="result.engine.wikidataId" class="subtitle is-6">
        Wikidata ID: {{ result.engine.wikidataId }}
      </p>
      ...
      <div class="columns is-multiline">
        <div
          v-for="game in result.engine.games.nodes"
          :key="game.id"
          class="column is-3"
        >
          <div class="card">
...
const { result, loading, error } = useQuery(GET_ENGINE, {
  id: route.params.id,
})
</script>
